...
const LightIndicator: React.FC<LightIndicatorProps> = ({ isOn }) => {
  ...
  useEffect(() => {
    ...
    const filament = svg.querySelector('.filamento');
    const halo = svg.querySelector('.halo');

    if (filament && halo) {
      if (isOn) {
        // Estado encendido
        filament.classList.add('filam-animation');
        halo.classList.add('hal-animation');
      } else {
        // Estado apagado
        filament.classList.remove('filam-animation');
        halo.classList.remove('hal-animation');
        filament.setAttribute('fill', '#777');
        halo.setAttribute('style', 'opacity: 0; transform: scale(0.2);');
      }
    }
  }, [isOn]);

  return (
    <div className="light-indicator-wrapper">
      <svg
        ref={svgRef}
        viewBox="0 0 100 200"
        width="100%"
